...
describe('parseDkimTxt', () => {
  it('v=DKIM1; k=ed25519; p=... から 32 バイト公開鍵を取り出す', () => {
    const result = parseDkimTxt(`v=DKIM1; k=ed25519; p=${KEY32}`);
  ...
  it('k が ed25519 でなければ null を返す', () => {
    expect(parseDkimTxt(`v=DKIM1; k=rsa; p=${KEY32}`)).toBeNull();
  });

  it('公開鍵が 32 バイトでなければ null を返す', () => {
    const short = Buffer.from([1, 2, 3]).toString('base64');
    expect(parseDkimTxt(`v=DKIM1; k=ed25519; p=${short}`)).toBeNull();
  });

  it('p が無ければ null を返す', () => {
    expect(parseDkimTxt('v=DKIM1; k=ed25519')).toBeNull();
  });

  it('p のデコードに失敗したら null を返す', () => {
    expect(parseDkimTxt('v=DKIM1; k=ed25519; p=%%%')).toBeNull();
  });
});
